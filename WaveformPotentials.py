...
class WaveformPotentials23Elec:
    ...
    def generate(self,timeStep,vInit,vFinal,incplTime,outcplTime,maxAmp,decelDist,elecSelect):
        ...
        zeroel = zeros((6,1))
        potentialsOut = hstack((newPotIn, newPotChirp, newPotPostChirp, zeroel))

        # for clock bit in wf generator circuitry to have even number of samples
        if max(shape(potentialsOut))%2==1:
            potentialsOut = hstack((potentialsOut, zeroel))
        ...
        self.plotPotentials = nan_to_num(potentialsOut)

    def buildArray(self,deltaT,wfstartdelay,PEE1,PEE2,surf,mesh,comp,aperture,tend):
            ...
            surfaceBias = surf*ones((1, timeTotal))
            extractionMesh = mesh*ones((1, timeTotal))
