...
void set_pasize(unsigned int nx_l, unsigned int nr_l, double dx_l, double dr_l)
{
	nx = nx_l;
	nr = nr_l;
	dx = dx_l;
...
void add_pa(unsigned int n, double * potential, double voltage)
{
	pas[n].voltage = voltage;
	pas[n].potential = __builtin_assume_aligned(potential, 16);
	printf("adding electrode %d to accelerator\n", n);
}

void fastAdjustAll(double * voltage)
{
...
double getPotential(double x, double r)
{
...
static inline void getField(double x, double r, double * result)
{
...
void getSingleFieldGradient(double x, double r, double * result)
{
	double E0[2];
	...
	getField(x+hx, r, &dx2[0]);
	getField(x-hx, r, &dx1[0]);
	getField(x, r+hr, &dr2[0]);
	getField(x, r-hr, &dr1[0]);
	
	result[0] = (E0[0]*(dx2[0] - dx1[0]) + E0[1]*(dx2[1] - dx1[1]))/normE/dx;
	result[1] = (E0[0]*(dr2[0] - dr1[0]) + E0[1]*(dr2[1] - dr1[1]))/normE/dr;
}

void getFieldGradient(unsigned int nParticles, double * x, double * r, double * result)
{
