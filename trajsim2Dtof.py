...
class RSDTrajSim(object):
        ...
        def propagateAtoms(self, potentialPCB, n, deltaT, initnum):
        # propagate atoms with position Verlet
                ...
                recPart = np.empty([0,2])
                initPart = float(initnum)
                ...
                for s in np.arange(3, steps):
                        ...
                        belowPCB = (rxCurrent >= 20.1e-3) & (ryCurrent <= 8.2e-3)
                        ...
                        # choose cyinder along PCB surface as detectable at 0.1mus
                        if s % int(round(1E-7/deltaT)) == 0:
                            detectIndices = np.where((rxCurrent >= 29.9e-3) & (rxCurrent <= 51.4e-3) & (ryCurrent >= 8.3e-3) & (ryCurrent <= 12.2e-3))[0]
                            recPart = np.append(recPart, [[s*deltaT, len(detectIndices)/initPart]], axis=0)
...
if __name__ == '__main__':
        ...
        maxAmp = 25
        vInit = vBeamFwd
        vFinal = vInit/2.0 # just guiding, no acceleration
        # /mm and /mus
        ...
        for j in range(len(looparg)):
            ...
            recparts = propagator.propagateAtoms(wfarr[j], n, deltaT, propagator.initnum)
            np.savetxt('./dataout/TOFHeDecelAmp/prop_particles_' + gas + '_' + strftime('%y%m%d%H%M') + '_' + 'n' + str(pqn) + '_' + loopparam + '_' + str(looparg[j]), recparts, delimiter='\t', newline='\n')
